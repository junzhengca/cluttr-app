...
const CardContainer = styled(TouchableOpacity)<{
  ...
  background-color: ${({
    ...
    backgroundColor,
    theme,
  }: StyledPropsWith<{ isActive?: boolean; backgroundColor?: string }>) =>
    isActive
...
const Title = styled(Text)<{ isActive?: boolean }>`
  color: ${({ isActive, theme }: StyledPropsWith<{ isActive?: boolean }>) =>
...
const SwitchIconContainer = styled(View)`
  ...
  background-color: ${({ theme }: StyledProps) =>
    theme.colors.primaryExtraLight || 'rgba(0, 0, 0, 0.05)'};
  align-items: center;
  justify-content: center;
  margin-bottom: 4px;
`;
        ...
        <ContentContainer>
          ...
          <Subtitle isActive={isActive}>
            {[
              ...
              .filter(Boolean)
              .join(' · ') || (isActive ? t('share.home.currentHome') : '')}
          </Subtitle>
        </ContentContainer>
      </CardContainer>
