...
const ExpiryBadge = styled(View)<{ isExpired?: boolean; isNear?: boolean }>`
  ...
  background-color: ${({
    ...
    isExpired,
    isNear,
  }: StyledProps & { isExpired?: boolean; isNear?: boolean }) => {
...
const VendorBadge = styled(View)`
  background-color: ${({ theme }: StyledProps) =>
    theme.colors.background || '#F4F6F8'};
  ...
  const renderExpiryBadge = () => {
    ...
    return (
      <ExpiryBadge isExpired={isExpired} isNear={isNear}>
        <ExpiryText isExpired={isExpired} isNear={isNear}>
          {text}
        </ExpiryText>
      </ExpiryBadge>
    );
      ...
      <ContentWrapper>
        ...
        <RightContainer>
          ...
          <DetailsRow>
            {batch.purchaseDate && (
              <DetailText>
                {t('itemDetails.batch.inboundDate')}
                {batch.purchaseDate.split('T')[0]}
              </DetailText>
            )}
            <DetailText>
              {t('itemDetails.batch.expiryDate')}
              {batch.expiryDate ? batch.expiryDate.split('T')[0] : '-'}
            </DetailText>
            <DetailText>
              {t('itemDetails.batch.unitPrice')}
