...
class InvitationService {
    ...
    private async createInvitation(home: Home, ownerProfile: User): Promise<string> {
        ...
        await homeRef(home.id).update({ invitationCode: code, updatedAt: isoNow() });
        return code;
    }
     ...
     */
    async accept(code: string, uid: string): Promise<{ success: boolean; message?: string }> {
        try {
            const snapshot = await invitationRef(code.trim()).get();
            if (!snapshot.exists()) {
            ...
            await homeRef(homeId).update({
                [`members.${uid}`]: { role: 'member', joinedAt: isoNow(), inviteCode: code.trim() },
